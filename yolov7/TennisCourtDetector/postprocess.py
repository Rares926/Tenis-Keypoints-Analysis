...
try:
    from TennisCourtDetector.utils import line_intersection
except:
...
def refine_kps(img, x_ct, y_ct, crop_size=40):
    """
    Refine the position of a keypoint in an image using the intersections of detected lines.

    Method info:
        This function takes an image and the current x and y coordinates of a keypoint.
        It creates a cropped image around the keypoint based on a specified crop size.
        It then detects lines within the cropped image.
        If multiple lines are detected, they are merged to simplify the detection.
        If exactly two lines remain after merging, their intersection is calculated.
        If a valid intersection point is found within the cropped image,
            the keypoint coordinates are updated to this intersection point.
        The updated coordinates are transformed back into the original image's coordinate
            space and returned.

    Parameters:
    - img (ndarray): The image in which the keypoint is located.
    - x_ct (int): The x-coordinate of the keypoint in the image.
    - y_ct (int): The y-coordinate of the keypoint in the image.
    - crop_size (int, optional): The size of each side of the
        square crop around the keypoint. Defaults to 40.

    Returns:
    - tuple of int: The refined y and x coordinates of the keypoint in the original image.
